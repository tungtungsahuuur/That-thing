local function LoadAdvancedConsole()
	...
	function AdvancedConsole:_createMessageContainer()
		...
		self._messageContainer.CanvasPosition = Vector2.new(0, self._messageContainer.CanvasSize.Y.Offset) -- Start at bottom
		...
		-- Connect to automatically scroll to bottom when new messages are added
		uiListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			self._messageContainer.CanvasPosition = Vector2.new(0, self._messageContainer.CanvasSize.Y.Offset)
		end)
	end
	...
	function AdvancedConsole:ExecuteCommand(commandText)
		...
		-- Log the command that was entered as Info type
		self:Log("> " .. commandText, "Info")
	...
	function AdvancedConsole:Log(text, messageType)
		messageType = messageType or "Normal"
		return self:_addMessage(messageType, text)
	end
